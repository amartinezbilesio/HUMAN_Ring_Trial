...
- the data has been filtered to include only the m/z range of 100 - 1100 Da
...
if (!file.exists(file.path(base_path, "object/sp_full.RData"))) {
  ...
  sp_full <- filterMzRange(sp_full, mz = c(100, 1100))
...
```{r load-annotations}
study_group <- "HE"
res_afekta <- read.csv(here("4_library_generation", "afekta",
                                 study_group, "ring_trial_library_HE.csv"))
res_hmgu <- read.csv(here("4_library_generation", "hmgu",
                               study_group, "ring_trial_library_HE.csv"))
res_icl <- read.csv(here("4_library_generation", "icl",
                               study_group, "ring_trial_library_HE.csv"))
res_cembio <- read.csv(here("4_library_generation", "cembio",
                                 study_group, "ring_trial_library_HE.csv"))
```
...
if (!file.exists(file.path(base_path, "object/sp_full_ann.RData"))) {
  ...
  sp_full_ann <- filterMzRange(sp_full_ann, mz = c(100, 1100))
  sp_full_ann <- setBackend(sp_full_ann, MsBackendMemory())
  sp_full_ann <- applyProcessing(sp_full_ann)
...
This is currently being done in a separate file. in the end i will probably
remove the code below. It is a bit more complicated than planed.

```{r peak-table-detected, eval=FALSE}
#' where can i extract the chrompeaks info
cpd_afekta <- read.csv(file.path(
  base_path,
  "object",
  "detected_peaks_afekta_HE.csv"
))
cpd_hmgu <- read.csv(file.path(
  base_path,
  "object",
  "detected_peaks_hmgu_HE.csv"
))
cpd_icl <- read.csv(file.path(base_path, "object", "detected_peaks_icl_HE.csv"))
cpd_cembio <- read.csv(file.path(
  base_path,
  "object",
  "detected_peaks_cembio_HE.csv"
))
...
colnames(peak_table_detect)[1:4] <- c("rtMin", "rtMax", "mzMin", "mzMax")
...
```{r load-eics-detected, eval=FALSE}
#| eval: false
if (!file.exists(file.path(base_path, "object/eics_detected.RData"))) {
  ...
  ## maybe we don't impute for now. it's so much data honestly. We cna filter out to get a smaller set before.  
