...
for (f in files) if (!file.exists(f)) stop("Missing input file: ", f)
...
peaks_mid <- lapply(dt_list, get_mid)

# BiocParallel parameter
BPPARAM <- MulticoreParam(workers = cores)

compute_mclosest_chunks <- function(from_lab, to_lab, chunk_size, peak_cols = 1:4) {
  df_from <- dt_list[[from_lab]]
  df_to   <- dt_list[[to_lab]]
  n_from <- nrow(df_from)
  if (n_from == 0 || nrow(df_to) == 0) return(data.table(from = integer(), to = integer()))

  chunks <- split(seq_len(n_from), ceiling(seq_len(n_from) / chunk_size))

  res_chunks <- bplapply(chunks, function(idx_chunk) {
...
all_nodes <- unlist(lapply(labs, function(lb) paste0(lb, ":", seq_len(nrow(dt_list[[lb]])))))
...
consensus_rows <- lapply(seq_len(nrow(consensus_list)), function(i) {
  ...
  members <- row$idxs[[1]]
  # pick first index per lab (could be extended to list-columns)
  idx_afekta <- if ("afekta" %in% members$lab) members[idx == min(idx) & lab == "afekta"]$idx[1] else NA_integer_
  idx_hmgu   <- if ("hmgu"   %in% members$lab) members[idx == min(idx) & lab == "hmgu"]$idx[1]   else NA_integer_
  idx_icl    <- if ("icl"    %in% members$lab) members[idx == min(idx) & lab == "icl"]$idx[1]    else NA_integer_
  idx_cembio <- if ("cembio" %in% members$lab) members[idx == min(idx) & lab == "cembio"]$idx[1] else NA_integer_

  # compute consensus mz/rt as mean of mids
  mz_vals <- c()
  rt_vals <- c()
  if (!is.na(idx_afekta)) { mz_vals <- c(mz_vals, peaks_mid$afekta$mz[idx_afekta]); rt_vals <- c(rt_vals, peaks_mid$afekta$rt[idx_afekta]) }
  if (!is.na(idx_hmgu))   { mz_vals <- c(mz_vals, peaks_mid$hmgu$mz[idx_hmgu]);   rt_vals <- c(rt_vals, peaks_mid$hmgu$rt[idx_hmgu]) }
  if (!is.na(idx_icl))    { mz_vals <- c(mz_vals, peaks_mid$icl$mz[idx_icl]);      rt_vals <- c(rt_vals, peaks_mid$icl$rt[idx_icl]) }
  if (!is.na(idx_cembio)) { mz_vals <- c(mz_vals, peaks_mid$cembio$mz[idx_cembio]); rt_vals <- c(rt_vals, peaks_mid$cembio$rt[idx_cembio]) }

  consensus_mz <- if (length(mz_vals) > 0) mean(mz_vals, na.rm = TRUE) else NA_real_
  consensus_rt <- if (length(rt_vals) > 0) mean(rt_vals, na.rm = TRUE) else NA_real_
...
consensus_dt[, chrom_peak_id_afekta := ifelse(!is.na(idx_afekta), dt_list$afekta$chrom_peak_id[idx_afekta], NA_character_)]
consensus_dt[, chrom_peak_id_hmgu := ifelse(!is.na(idx_hmgu), dt_list$hmgu$chrom_peak_id[idx_hmgu], NA_character_)]
consensus_dt[, chrom_peak_id_icl := ifelse(!is.na(idx_icl), dt_list$icl$chrom_peak_id[idx_icl], NA_character_)]
consensus_dt[, chrom_peak_id_cembio := ifelse(!is.na(idx_cembio), dt_list$cembio$chrom_peak_id[idx_cembio], NA_character_)]

# Add consensus id
consensus_dt[, consensus_id := .I]

# Reorder columns
setcolorder(consensus_dt, c("consensus_id", "consensus_mz", "consensus_rt", "n_labs_matched",
                           "idx_afekta", "chrom_peak_id_afekta",
                           "idx_hmgu", "chrom_peak_id_hmgu",
                           "idx_icl", "chrom_peak_id_icl",
                           "idx_cembio", "chrom_peak_id_cembio", "comp"))
...
fwrite(edges, paste0(out_prefix, "_edges.csv"))

message("Done. Consensus rows: ", nrow(consensus_dt))
