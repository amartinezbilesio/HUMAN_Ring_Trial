...
- USE TARDIS low level functions for peak detection, metrics calculation, smoothing ?, and flagging bad eics 
...
Below need to first load the chrompeaks table, 
then find a set of consensus peaks across all labs.
johannes sugest to use mclosest to create some sort of compatibility matrice at first 
then filter for the chrompeaks that have no NAs (if we filter out too much then keep 3/4 instead)

extract eics from it and then 
add annotation info to the chromData as extract columns and mapping using chrompeakID. 
...
- plot both the detected (total) and annotated (using a columns of the chromData)
