...
writes `results_all_ms2/annotation_chrom_metrics.csv` directly; the
bulk annotated TIC (P4) is `sum(areaUnderTic)` per `(lab, mixture,
polarity)` over that table, computed inline in
`8_data_level_progression.qmd`. See
[`CANONICAL_SCHEMAS.md`](CANONICAL_SCHEMAS.md).

# Summary

Public objects (consumed by `8_data_level_progression.qmd`,
`2_consensus_peaks.qmd`):
...
`results_all_ms2/annotation_chrom_metrics.csv` — consumers derive bulk
